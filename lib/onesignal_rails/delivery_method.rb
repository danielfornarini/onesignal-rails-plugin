...
module OneSignalRails
  ...
  class DeliveryMethod
    ...
    def deliver!(mail)
      ...
      notification = OneSignal::Notification.new({ app_id: settings[:app_id] || OneSignalRails.configuration.app_id,
                                                   is_ios: false })
    ...
    def validate(mail)
      ...
      if (mail.subject == USE_TEMPLATE_SUBJECT) && !mail['template_id']
        raise 'Must specify template_id if setting subject to OneSignalRails::USE_TEMPLATE_SUBJECT!'
      end
    end

    def add_from(notification, mail)
      from_email = to_email(mail.from)
    ...
    def add_to(notification, mail)
      # a specified "to override" takes priority over any to provided within the mail
    ...
    def add_custom_notification_args(notification, mail)
      ...
      if input.is_a?(Mail::Field)
        ...
        raise OneSignalDeliveryError, "Unknown type for customer_notification_args: #{input.class.name}"
      end
    end
    ...
    def client
      ...
      config.configure do |c|
        c.app_key = settings[:app_key] || OneSignalRails.configuration.app_key
      end
    ...
    def send(notification)
      ...
      raise OneSignalDeliveryError.new('OneSignal request responded with errors', response) if response.errors
    end
  end
end
