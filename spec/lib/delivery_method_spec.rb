    ...
    describe '#deliver!' do
      ...
      context 'when sending email subject' do
        ...
        it 'fails if told not to provide subject but no template is used' do
          mail.subject = OneSignalRails::USE_TEMPLATE_SUBJECT
          expect do
            mailer.deliver!(mail)
          end.to raise_exception('Must specify template_id if setting subject to OneSignalRails::USE_TEMPLATE_SUBJECT!')
        end
      end
